...
class JRUtilities {   
    ...
    public function wakeUpIncidentByProcessId($processId) { return $this->db->wakeUpIncidentByProcessId($processId); }
    public function wakeUpIncidentByProcessName($processName) { return $this->db->wakeUpIncidentByProcessName($processName); }
...
class JRData {
	...
	public static function toAmount($numRaw, $toStr = false, $separator = '.') {
        ...
        //error_log("num: $numRaw, negative: $isNegative, result: $result");
...
class JRDatabase {
	...
	public function query($sqlRaw, $dbName = 'jr') {
	...
	public function wakeUpIncidentByProcessName($processName) {
        $sql = "
            UPDATE jrincidents SET step_status = 99 
            OUTPUT inserted.* 
            WHERE processname = '$processName' AND step_status = 0 AND module_name = 'wait'
        ";
...
class JRLogger {
    ...
    private function attachLogFileToJRDatabase() { $u = $this->u; $src = $this->src;
        $sql = ($u->process['dbType'] === 1)
            ...
            "
            : "
                SELECT * FROM JRATTACHMENTS WHERE processid = '{$this->processId}' AND filename = '{$this->logFile}';
                IF @@ROWCOUNT = 0
                INSERT INTO JRATTACHMENTS (processid, folder, filename, original_filename)
        		VALUES ('{$this->processId}', '$this->logFolderDbEntry', '{$this->logFile}', '{$this->logFile}');
            ";
...
class JRIncident {
    ...
    public function st($params) { $this->data["subtable_insstring"] = $params; return $this; }
    ...
    public function stepEscalationDate($stepEscalationDate) { $this->data["step_escalation_date"] = $stepEscalationDate; return $this; }
    public function incidentEscalationDate($incidentEscalationDate) { $this->data["incident_escalation_date"] = $incidentEscalationDate; return $this; }

<?php
// utils_php build 0119

declare(strict_types=1);
set_time_limit(30);
//error_reporting(-1);

class JRUtilities {   
    private $env = [];
    private $dateUtils;

    public function __construct($source) {
        $this->env['u'] = $this;
        $this->env['src'] = $source;
        $this->env['dbMap'] = null;
        $this->env['logger'] = new JRLogger($this, $source, 'DEBUG', 'FILE');
        $this->env['dbMap'] = $this->loadDatabaseMap();
    }

    private function loadDatabaseMap() { extract($this->env);
        $processName = $src->getProcessName();

        $sql = "
            SELECT subtable, field_name as field, field_type as type FROM jrsubtablefields WHERE processname = '$processName'
            UNION
            SELECT 'pt' as subtable, field_name as field, field_type as type FROM jrtablefields WHERE processname = '$processName'
        ";

        return $u->query($sql);
    }

    public function log(LogLevel $level, string $msg, $type = LogType.STR) { extract($this->env);
	    $logger->log($level, $msg, $type); 
	}

	public function resetlogFile() { extract($this->env);
        $logger->createNewLogFile(); 
    }

    public function initLogger(LogLevel $logLevel, LogChannel $logChannel) { extract($this->env);
        $logger->setChannel($logChannel);
        $logger->setLevel($logLevel);
    }

    public function getValue(string $target) {
        if (strpos($target, '.') !== false) {
            $targetAry = explode('.', $target);
            $type = $this->getTargetType($targetAry[0], $targetAry[1]);

            $result = ($targetAry[2] === '*')
                ? $this->getIncidentArray($target, $targetAry, $type)
                : $this->getIncidentValue($target, $targetAry, $type);
        }
        else {
            $type = $this->getTargetType('pt', $target);
            $result = $this->getIncidentValue($target, null, $type);
        }
        
        return $result;
    }

    private function getTargetType(string $table, string $field) : FieldType { extract($this->env);
        foreach ($dbMap as $item) {
            if ($item['subtable'] === $table && $item['field'] === $field) {
                switch ($item['type']) {
                    case "varchar" : return FieldType.TEXT;
                    case "int" : return FieldType.INTEGER;
                    case "decimal" : return FieldType.FLOAT;
                    case "datetime" : return FieldType.DATETIME;
                    case "file" : return FieldType.ATTACHMENT;
                }
            } 
        }

        throw new Exception("Target '$table.$field' does not exist.");
    }

    private function getIncidentValue(string $target, array $targetAry, FieldType $type) { extract($this->env);
        $value = (isset($targetAry))
            ? $src->getSubtableValue($targetAry[0], intval($targetAry[2]), $targetAry[1], true)
            : $src->getTableValue($target, true);
        
        if (empty($value)) return null;

        $typedValue = null;

        switch($type) {
            case FieldType.TEXR:      $typedValue = $value; break;
            case FieldType.INTEGER:     $typedValue = $this->toInt($value); break;
            case FieldType.FLOAT:       $typedValue = $this->toFloat($value); break;
            case FieldType.DATETIME:    $typedValue = $this->toDate($value); break;
            case FieldType.ATTACHMENT:  $typedValue = $value; break;
            default: throw new Exception("unknown type '$type' at '$target");
        }

        return $typedValue;
    }

    private function getIncidentArray(string $target, array $targetAry, string $type) : array { extract($this->env);
        $subtableRowIds = $src->getSubtableRowIds($target[0]);
        $result = array();

        foreach ($subtableRowIds as $row) {
            $targetAry[2] = $row;
            $value = $this->getIncidentValue($target, $targetAry, $type);
            array_push($result, $value);
        }

        return $result;
    }
    
    public function setValue(string $target, $value) { extract($this->env);
        if (strpos($target, '.') !== false) {
            $targetAry = explode('.', $target);

            if ($targetAry[2] === '*') $this->setIncidentArray($targetAry, $value);
            else $this->setIncidentValue($targetAry, $value);
        }
        else $this->setIncidentValue($target, $value);
    }
    
    private function setIncidentValue(string $target, $value) { extract($this->env);
        if (is_array($target)) $src->setSubtableValue($target[0], intval($target[2]), $target[1], $value);
        else $src->setTableValue($target, $value);
    }

    private function setIncidentArray(string $target, $value) { extract($this->env);
        $subtableRowIds = $src->getSubtableRowIds($target[0]);

        foreach ($subtableRowIds as $row) {
            $target[2] = $row;
            $this->setIncidentValue($target, $value);
        }
    }

    public function insertStRow(string $subtable, $rowData, bool $prepend) { extract($this->env);
        $rowId = ($prepend) ? 1: intval($src->getSubtableCount($subtable)) + 1;
        $src->insertSubtableRow($subtable, $rowId, $rowData);
    }

    public function filterArray(array $srcArray, array $filters, bool $distinct) {
        $tarArray = array();
        $values = array();

        if ($filters === null) return $srcArray;

        foreach ($srcArray as $key => $value) {
            if (in_array($key, $filters) || ($distinct && in_array($value, $values))) continue;
            $tarArray[$key ] = $value;
            array_push($values, $value);
        }
        
        return $tarArray;
    }
	
	public function toAmount(string $numRaw, bool $toStr = false, string $separator = '.') {
	    $num = strval($numRaw);
	    $isNegative = (strpos($num, '-') === 0) ? true : false;
	    
        $dotPos = strrpos($num, '.');
        $commaPos = strrpos($num, ',');
        $sep = (($dotPos > $commaPos) && $dotPos) ? $dotPos :
            ((($commaPos > $dotPos) && $commaPos) ? $commaPos : false);
      
        $result = (!$sep) ? 
            preg_replace("/[^0-9]/", '', $num) :
            preg_replace("/[^0-9]/", '', substr($num, 0, $sep)) . $separator .preg_replace("/[^0-9]/", '', substr($num, $sep+1, strlen($num)));
        
        if ($isNegative) $result = '-' . $result;
        
        return ($toStr) ? $result : floatval($result);
    }
    
    public function getType($value, bool $returnClass = true) { 
        if (isset($value)) {
            $baseType = gettype($value);
            $resultType = ($baseType === 'object' && $returnClass) ? get_class($value) : $baseType;
            $resultTypeNormalized = strtolower(trim($resultType));
        }
        else return 'string';
        
        return $resultTypeNormalized;
    }

    public function removeLeadingZeroes(string $str) : string { 
        return ltrim($str, '0'); 
    }

    public function addLeadingZeroes(string $str, int $strLength) : string { 
        return str_pad($str, $strLength, '0', STR_PAD_LEFT); 
    }

    public function toInt($val): int {
        if (!$this->safeInt($val)) throw new Exception('Value could not be converted to int');
        else return (int)$val;
    }
    
    public function toFloat($val): float {
        if (!$this->safeFloat($val)) throw new Exception('Value could not be converted to float');
        else return (float)$val;
    }
    
    public function toString($val): string {
        if (!$this->safeString($val)) throw new Exception('Value could not be converted to string');
        else return (string)$val;
    }

    public function toDate($val): DateTime { 
        return new DateTime($val); 
    }

    public function toPath(string $path) : string {
        $path = str_replace( '\\', '/', $path );
        $path = preg_replace('|(?<=.)/+|', '/', $path);
        return ( ':' === substr( $path, 1, 1 ) ) ? ucfirst( $path ) : $path;
    }
    
    public function toSql(array $data) : array { extract($this->env);
        if (!isset($data)) throw new Exception('data is not set');
        
        $sqlData = [];
        
        foreach($data as $key => $value) {
            if (empty($value)) {
                $sqlData[$key] = 'NULL';
                continue;
            }

            $type = $u->getType($value);
            
            switch ($type) {
                case 'string': $result = "'$value'"; break;
                case 'datetime': $result = "'" . $value->format('Y-m-d H:i:s') . "'"; break;
                default: $result = $value;
            }
            
            $sqlData[$key] = $result;
        }
        
        return $sqlData;
    }

    private function safeInt($val) {
        switch (gettype($val)) {
            case 'integer': return true;
            case 'double': return $val === (float)(int)$val;
            case 'string':
                $losslessCast = (string)(int)$val;
                if ($val !== $losslessCast && $val !== "+$losslessCast") return false;
                return $val <= PHP_INT_MAX && $val >= PHP_INT_MIN;
            default: return false;
        }
    }
    
    private function safeFloat($val) {
        switch (gettype($val)) {
            case 'double':
            case 'integer': return true;
            case 'string':
                if (strlen($val) > 1 && $val[0] === '0' && $val[1] !== '.') return false;
    
                $lnum    = "[0-9]+";
                $dnum    = "([0-9]*[\.]{$lnum})|({$lnum}[\.][0-9]*)";
                $expDnum = "/^[+-]?(({$lnum}|{$dnum})[eE][+-]?{$lnum})$/";
    
                return preg_match("/^[+-]?{$lnum}$/", $val) || preg_match("/^[+-]?{$dnum}$/", $val) || preg_match($expDnum, $val);
            default: return false;
        }
    }
    
    private function safeString($val) {
        switch (gettype($val)) {
            case 'string':
            case 'integer':
            case 'double': return true;
            case 'object': return method_exists($val, '__toString');
            default: return false;
        }
    }

    public function query(string $sqlRaw, string $dbName = 'jr') { extract($this->env);
		$db = ($dbName === 'jr') ? $src->getJobDB() : $src->getDBConnection($dbName);
		
		$sql = str_replace(['\r','\n','\t','   ', '  '], ['','','',' ',' '], $sqlRaw);

        $result = $db->query($sql);

        if (!empty($db->getErrorMessage())) {
            $db->free();
            throw new Exception($db->getErrorMessage());
        }

        $returnValue = $db->fetchAll($result);
        $db->free();

        return $returnValue;
	}
	
	public function sQuery(string $sql, array $params, string $dbName = 'jr') { extract($this->env);
		$db = ($dbName === 'jr') ? $src->getJobDB() : $src->getDBConnection($dbName);
		
        $result = (strtolower(strtok(trim($sql), ' ')) === 'select') 
            ? $db->preparedSelect($sql, $params)
            : $db->preparedExecute($sql, $params);

        if (!empty($db->getErrorMessage())) {
            $db->free();
            throw new Exception($db->getErrorMessage());
        }
            
        $returnValue = $db->fetchAll($result);
        
        $db->free();

        return $returnValue;
	}

    public function getUser(string $user, array $roleFilter): JRUser {
        $sql = "SELECT u.username, u.email, u.lastname, u.prename FROM jrusers AS u WHERE u.username = '$user'";

        $result = $this->query($sql);

        if (sizeof($result) === 0) return null;

        return new JRUser($result['username'], $result['email'], $result['lastname'], $result['prename']); 
    }

    private function getUsersByRole(string $role) : array {
        $sql = "
            SELECT u.username, u.email, u.lastname, u.prename 
            FROM jrusers AS u JOIN jruserjob as j ON u.username = j.username 
            WHERE j.jobfunction = '$role'
        ";

        $results = $this->query($sql);

        if (sizeof($results) === 0) return null;

        $users = [];

        foreach($results as $result) {
            $user = new JRUser($result['username'], $result['email'], $result['lastname'], $result['prename']);
            array_push($users, $user);
        }

        return $users; 
    }

    public function getRolesByUser(string $user) : array {
        $sql = "SELECT j.jobfunction FROM jrusers AS u JOIN jruserjob as j ON u.username = j.username WHERE u.username = '$user'";
        return $this->query($sql);
    }
    
    public function wakeUpIncidentByProcessId(string $processId) : void {
        $sql = "UPDATE jrincidents SET step_status = 99 WHERE processid = '$processId' AND step_status = 0 AND module_name = 'wait'";
        $this->query($sql);
	}
	
	public function wakeUpIncidentByProcessName(string $processName) : void {
        $sql = "UPDATE jrincidents SET step_status = 99 WHERE processname = '$processName' AND step_status = 0 AND module_name = 'wait'";
        $this->query($sql);
	}

    public function copyAttachmentToPath(string $target) : void { extract($this->env);
        $srcPath = $u->getValue($src);
        $srcPathNormalized = $u->toPath($srcPath);
        copy($srcPath , $target);
	}

    /* JRMail*/
    public function buildMail() : JRMail { return new JRMail($this); }

    /* JRIncident */
    public function buildIncident() : JRIncident { return new JRIncident($this); }
}

class JRLogger {
    private $env, $logFolder, $logFolderDbEntry, $logFile, $logFilePath;
    private $processId, $processName, $processVersion, $fullUploadPath;

    public function __construct(JRUtilities $utils, object $source) {
        $this->env = [
            'u' => $utils,
            'src' => $source,
            'logCfg' => [
                'channel' => $logChannel, // [PT|FILE]
                'levels' => array('VERBOSE' => 0, 'DEBUG' => 1, 'INFO'=> 2, 'WARN' => 3, 'ERROR' => 4, 'SUCCESS' => 5),
                'level' => 1, // (DEBUG => 1, INFO => 2, WARN => 3, ERROR => 4)
                'style' => 'HTML', // [HTML|TXT]
                'field' => 'a_log'
            ]
        ];

        extract($this->env);

        $this->initConfig();
        $this->attachLogFileToJRDatabase();
		$src->setTableValue($logCfg['field'], "{$this->logFolderDbEntry}/{$logFile}");
    }

    public function setChannel(LogChannel $logChannel) { extract($this->env);
        $logCfg['channel'] = $logChannel;
    }

    public function setLevel(LogLevel $logLevel) { extract($this->env);
        $logCfg['level'] = $logLevel;
    }
    
    private function initConfig() { extract($this->env);
        $this->processId = $src->getProcessId();
        $this->processName = $src->getProcessName();
        $this->processVersion = $src->getVersion();
        $this->fullUploadPath = $src->getFullUploadPath();
        $processIdTrimmed = substr($this->processId, -10);
        $this->logFolder = "log\\$processIdTrimmed\\";
        $this->logFolderDbEntry = "log/$processIdTrimmed";
		$this->logFile = $this->processId . '.log.html';
        $this->logFilePath = "{$this->fullUploadPath}{$this->logFolder}{$this->logFile}";
    }
    
    public function log(LogLevel $logLevel, string $msg, LogType $type) { extract($this->env);
        $caller = $this->getCaller();
        $htmlEncodedMsg = $this->getHtmlEncodedMsg($msg, $type);

        if ($this->isLogAcceptedByLogLevelFilter($logLevel)) {
            switch($logCfg['channel']) {
                case 'PT': $this->logInHtmlStyle($logLevel, $htmlEncodedMsg, $caller); break;
                case 'FILE':  error_log("$logLevel | $caller -> $htmlEncodedMsg"); break;
                default: error_log("unknown logChannel {$logCfg['channel']}"); break;
            }
        }
    }

    private function logInHtmlStyle(LogLevel $logLevel, LogType $msg, string $caller) { extract($this->env);
        $logFieldContent = $src->getTableValue($this->logField);

        if (empty($logFieldContent) || $logFieldContent === '') $this->createNewLogFile();

        $current_date = $u->getDate('NOW', true);
        
        $data = "<p>$current_date, <span class=\"LOGLEVEL $logLevel\">$logLevel</span>, <span class=\"LOGFUNC\">$caller</span><b> -> </b>$msg</p>\n";
        $result = file_put_contents($this->logFilePath, $data, FILE_APPEND | LOCK_EX);
        if (!$result) { throw new Exception("writing to logfile {$this->logFilePath} failed."); }
    }
    
    private function getHtmlEncodedMsg(string $msg, LogType $type) : string {
        switch($type) {
            case LogType.STR: return $msg;
            case LogType.OBJ: return print_r($msg, true);
            case LogType.XML: return str_replace(array('<', '>'), array('&#60;', '&#62;'), $this->getFormattedXML($msg));
            default: throw new Exception("wrong type '$type'.");
        }
    }
    
    public function getFormattedXML(string $msg) : DOMDocument {
        $dom = new DOMDocument;
        $dom->preserveWhiteSpace = FALSE;
        $dom->loadXML($msg);
        $dom->formatOutput = TRUE;
        return $dom->saveXML();
    }

    private function getCaller() : string {
        $traceLine = ((new Exception)->getTrace())[3];
        $search = "{$this->processName}_{$this->processVersion}_";
        $callerClass = str_replace($search, '', $traceLine['class']);
        $callerFunction = $traceLine['function'];
        $result = "$callerClass::$callerFunction()";
        return $result;
    }

    private function attachLogFileToJRDatabase() { extract($this->env);
        $sql = "SELECT * FROM JRATTACHMENTS WHERE processid = '{$this->processId}' AND filename = '{$this->logFile}'";
        $result = $u->query($sql);

        if (sizeof($result) === 0) {
            $sql = "
                INSERT INTO JRATTACHMENTS (processid, folder, filename, original_filename)
                VALUES ('{$this->processId}', '$this->logFolderDbEntry', '{$this->logFile}', '{$this->logFile}');
            ";

            $u->query($sql);
        }
	}
	
	public function createNewLogFile() {
        $header = 
            '<meta http-equiv="refresh" content="15"/>'
            . "<style> "
                . "p { font-size:16px;font-family:helvetica;white-space: pre-wrap; } "
                . ".LOGLEVEL { font-size:13px; font-weight: bold; } "
                . ".SUCCESS { color: darkgreen; } "
                . ".ERROR { color: red; } "
                . ".WARN { color: goldenrod; } "
                . ".INFO { color: grey; } "
                . ".DEBUG { color: blue; } "
                . ".VERBOSE { color: blue; } "
                . ".LOGFUNC { color: deepskyblue; } "
            . "</style>\n";

        $path = "{$this->fullUploadPath}{$this->logFolder}";
        if (!file_exists($path)) { mkdir($path, 0777, true); }
        
        $result = file_put_contents($this->logFilePath, $header, LOCK_EX);
        if (!$result) { throw new Exception("Creating logfile {$this->logFilePath} failed."); }
	}

    private function isLogAcceptedByLogLevelFilter(LogLevel $logLevel) {
        $myLogLevel = $this->logLevels[$logLevel];
        return ($myLogLevel >= $this->logLevel) ? true : false;
	}
}

class JRIncident {
	private $env;
    private $data  = [
        'processname' => null,
        'initiator' => 'admin',
        'simulation' => 0,
        'version' => 0,
        'step' => null,
        'insstring' => null,
        'subtable_insstring' => null,
        'jobfunction' => null,
        'username' => null,
        'step_escalation_date' => null,
        'incident_escalation_date' => null,
        'indate' => null
    ];

    public function __construct(JRUtilities $utils) { $this->env = [ 'u' => $utils ]; }
    
    public function processname(string $processname) { $this->data['processname'] = $processname;  }
    public function version(int $version) { $this->data['version'] = $version; }
    public function initiator(string $initiator) { $this->data['initiator'] = $initiator; }
    public function step(int $step) { $this->data['step'] = $step; }
    public function simulation(bool $simulation) { $this->data['simulation'] = $simulation; }
    public function pt(array $params) { $this->data['insstring'] = $this->mappedImplode($params); }
    public function st(array $params) { $this->data['subtable_insstring'] = $params; }
    public function role(string $jobfunction) { $this->data['jobfunction'] = $jobfunction; }
    public function user(string $username) { $this->data['username'] = $username; }
    public function indate(DateTime $indate) { $this->data['indate'] = $indate; }
    public function stepEscalationDate(DateTime $stepEscalationDate) { $this->data['step_escalation_date'] = $stepEscalationDate; }
    public function incidentEscalationDate(DateTime $incEscalationDate) { $this->data['incident_escalation_date'] = $incEscalationDate; }
    
    private function mappedImplode(array $values, string $glue = ';', string $symbol = '=') : string {
        $func = function($k, $v) use($symbol) { return $k . $symbol . $v; };
        return implode($glue, array_map($func, array_keys($values), array_values($values)));
    }
    
    public function send() : void { extract($this->env);
        $sql = "
            INSERT INTO jrjobimport (
                processname, initiator, simulation, version, step, insstring, subtable_insstring, 
                jobfunction, username, step_escalation_date, incident_escalation_date, indate
            )
            VALUES (
                :processname, :initiator, :simulation, :version, :step, :insstring, :subtable_insstring, :jobfunction, 
                :username, :step_escalation_date, :incident_escalation_date, :indate
            )
        ";
        
        $u->sQuery($sql, $this->data);
    }
}

class JRMail {
    private $env;
    private $data = [
        'subject' => null,
        'msg' => null,
        'attachment' => null,
        'cc' => null,
        'bcc' => null,
        'indate' => null
    ];

    public function __construct(JRUtilities $utils) { $this->env = [ 'u' => $utils ]; }

    public function subject(string $subject) { $this->data['subject'] = $subject; }
    public function msg(string $msg) { $this->data['msg'] = $msg; }
    public function attachment(string $attachment) { $this->data['attachment'] = $attachment; }
    public function cc(string $cc) { $this->data['cc'] = $cc; }
    public function bcc(string $bcc) { $this->data['bcc'] = $bcc; }
    public function indate(DateTime $indate) { $this->data['indate'] = $indate; }

    public function target(string $toEmail, string $toName = null) { 
        $this->data['toEmail'] = $toEmail; 
        $this->data['toName'] = $toName; 
    }

    public function source(string $fromEmail, string $fromName = null) { 
        $this->data['fromEmail'] = $fromEmail; 
        $this->data['fromName'] = $fromName; 
    }

    public function send() : void { extract($this->env);
        //$p = $u->toSql($this->mail);

        if (empty($data['indate'])) {
            $this->data['indate'] = $u->getDate();
        }

        $sql = "
            INSERT INTO JRMAIL (
                from_email, from_name, to_email, to_name, cc_email, bcc_email, 
                subject, emailtext, attachment, indate, mailtype
            )
            VALUES (:fromEmail, :fromName, :toEmail, :toName, :cc, :bcc, :subject, :msg, :attachment, :indate, 1)
        ";
           
        $u->sQuery($sql, $this->data);
    }
}

class JRUser {
    public $username;
    public $email;
    public $lastname;
    public $prename;

    public function __construct($utils) { 
        $this->username = $username;
        $this->email = $email;
        $this->lastname = $lastname; 
        $this->prename = $prename;
    }
}

enum FieldType{ case TEXT; case INTEGER; case FLOAT; case DATETIME; case ATTACHMENT; }
enum LogLevel{ case DEBUG; case INFO; case WARN; case ERROR; }
enum LogType{ case STR; case OBJ; }
enum LogChannel{ case PT; case FILE; }

?>
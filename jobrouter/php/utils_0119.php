...
class JRUtilities {   
    ...
    public function log($level, $msg, $type = 'str') { extract($this->env);
    ...
    public function initLogger($logLevel, $logChannel) { extract($this->env);
    ...
    private function getTargetType($table, $field) { extract($this->env);
    ...
    private function getIncidentValue($target, $targetAry, $type) { extract($this->env);
    ...
    private function getIncidentArray($target, $targetAry, $type) { extract($this->env);
    ...
    public function setValue($target, $value) { extract($this->env);
        if (strpos($target, '.') !== false) {
            $targetAry = explode('.', $target);
    ...
    private function setIncidentValue($target, $value) { extract($this->env);
        if (is_array($target)) $src->setSubtableValue($target[0], intval($target[2]), $target[1], $value);
        else $src->setTableValue($target, $value);
    }

    private function setIncidentArray($target, $value) { extract($this->env);
        $subtableRowIds = $src->getSubtableRowIds($target[0]);
    ...
    public function insertStRow($subtable, $rowData, $prepend) { extract($this->env);
    ...
    public function toSql($data) { extract($this->env);
    ...
    public function query($sqlRaw, $dbName = 'jr') { extract($this->env);
	...
	public function sQuery($sql, $params, $dbName = 'jr') { extract($this->env);
		$db = ($dbName === 'jr') ? $src->getJobDB() : $src->getDBConnection($dbName);
    ...
    public function copyAttachmentToPath($target) { extract($this->env);
...
class JRLogger {
    ...
    public function __construct($utils, $source, $logChannel = 'PT', $logLevel = 1) {
        $this->env = [
            'u' => $utils,
            'src' => $source,
            'logCfg' => [
                ...
                'level' => $logLevel, // (DEBUG => 1, INFO => 2, WARN => 3, ERROR => 4)
                'style' => 'HTML', // [HTML|TXT]
                'field' => 'a_log'
            ]
        ];
    ...
    public function setChannel($logChannel) { extract($this->env);
        $logCfg['channel'] = $logChannel;
    }

    public function setLevel($logLevel) { extract($this->env);
    ...
    public function log($logLevelName, $msg, $type) { extract($this->env);
    ...
    private function logInHtmlStyle($logLevelName, $msg, $caller) { extract($this->env);
...
class JRIncident {
    ...
    public function __construct($utils) { $this->env = [ 'u' => $utils ]; }
    ...
    public function send() { extract($this->env);
        $sql = "
...
class JRMail {
    ...
    public function subject($subject) { $this->data['subject'] = $subject; }
    public function msg($msg) { $this->data['msg'] = $msg; }
    public function attachment($attachment) { $this->data['attachment'] = $attachment; }
    public function cc($cc) { $this->data['cc'] = $cc; }
    public function bcc($bcc) { $this->data['bcc'] = $bcc; }
    public function indate($indate) { $this->data['indate'] = $indate; }
